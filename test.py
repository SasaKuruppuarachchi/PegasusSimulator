...
class PX4LaunchTool:
    ...
    def launch_px4(self):
        ...
        if tmux_session_check.returncode == 0:
            self.px4_process = subprocess.Popen(['tmux', 'split-window', '-h', f'{self.px4_dir}/build/px4_sitl_default/bin/px4', f'{self.px4_dir}/ROMFS/px4fmu_common/', '-s', self.rc_script, '-i', str(self.vehicle_id), '-d'], cwd=self.root_fs.name, shell=False, env=self.environment)
        else:
            self.px4_process = subprocess.Popen([f'{self.px4_dir}/build/px4_sitl_default/bin/px4', f'{self.px4_dir}/ROMFS/px4fmu_common/', '-s', self.rc_script, '-i', str(self.vehicle_id), '-d'], cwd=self.root_fs.name, shell=False, env=self.environment)
        print(self.px4_process)
